...
**Contribution:**  
First conference presentation of the analytical KCL model. Demonstrates how power-loop inductance L_circ controls the dI/dt of the commutation current and thereby scales the positive crosstalk spike amplitude. Parametric study: L_circ = 60-320 nH at fixed R_g = 20 Ohm, V_bus = 400 V.
...
## [C2] EPE 2022: Conference Paper 2
...
2. **Optimal (R_g, L_e) design region** that minimizes worst-case spike amplitude for a given load current.
3. **Experimental validation** with a 400 V / 22 A double-pulse test bench using the IXGH60N60C2 IGBT at four gate resistance values (10, 15, 25, 70 Ohm) and four emitter inductance values (10, 23, 33, 52 nH).
...
3. **Phase 1/Phase 2 analytical expressions:** Closed-form symbolic solutions for rise time t_r and intermediate voltage V_a (Miller plateau phase), and for inductor current I_L(t) and V_CE(t) during the resonant fall phase.

> Code and simulation files for this work-in-preparation are maintained in a separate private repository. Contact the author for access.
