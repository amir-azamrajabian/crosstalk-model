...
| [Device Parameters](device-parameters.md) | Complete parameter tables for the IGBT (IXGH60N60C2) |
...
- There exists an **optimal (R_g, L_e) design point** that minimizes the worst-case spike while satisfying switching-loss constraints.
- The model has been experimentally validated on a 400 V / 22 A half-bridge using the IXGH60N60C2 IGBT. An extension to the IRFP450 Si MOSFET is in preparation for IEEE TPEL.
...
├── Publications/         <- Publication-specific scripts ([C1], [C2], [J1])
├── Simulations/PSpice/   <- PSpice project files (organized by paper)
└── docs/                 <- This Wiki
```
